...
func NewParcelServiceServer(db *gorm.DB, userService pb.UserServiceClient, sdClint *sd.ServiceDiscoveryClient) (pb.ParcelServiceServer, error) {
	err := db.AutoMigrate(&model.Parcel{})
	sdClint.ServiceRegistry(string(sd.ServiceName_PARCEL_SERVICE), "parcel", 4000, 4998)
	if err != nil {
